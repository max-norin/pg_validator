CREATE FUNCTION trigger_validate() RETURNS TRIGGER AS
$$
DECLARE
    "v"                        JSONB            = '{}';
    "column"                   TEXT;
        ...
        LOOP
            ...
            ELSEIF ("res" IS FALSE) THEN
                ...
                FOREACH "column" IN ARRAY array_unique("constraint"."columns")
                    LOOP
                        "v" = jsonb_array_append("v", ARRAY ["column"], '"exists"'::JSONB); -- to_jsonb('exists:' || "constraint"."name")
                    END LOOP;
            END IF;
        END LOOP;
    -- UNIQUE constraints
        ...
        LOOP
            ...
            ELSEIF ("res" IS FALSE) THEN
                FOREACH "column" IN ARRAY "constraint"."columns"
                    LOOP
                        "v" = jsonb_array_append("v", ARRAY ["column"], '"unique"'::JSONB); -- to_jsonb('unique:' || "constraint"."name")
                    END LOOP;
            END IF;
        END LOOP;
